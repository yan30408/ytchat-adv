    ...
    data['logs'].forEach( (value, index, array) => {
        ...
        // ユニット更新
          ...
          if(value.userId === userId){
            ...
            for(let i in nameList){
              if(nameList[i] && nameList[i].name === value.name && nameList[i].color !== value.color){
                nameList[i].color = value.color;
                npcBoxSet();
                break;
              }
            }
      ...
      if(value['system']){
        ...
        // ランダム表の定義
          ...
          value['comm'] += `（実行コマンド： <code>1$${m[1]}</code> ）`;
        }
        // レディチェック
        else if (value['system'].match(/^ready/)){
