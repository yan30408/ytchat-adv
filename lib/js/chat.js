...
function saveRoomConfig(){
  ...
  console.log('saveRoomConfig()',roomConfigAll);
...
async function readyCheckSet(firstLoaded){
  ...
  if(max === okNum + noNum){
    ...
    newLog.innerHTML = `<dt></dt><dd class="comm"><button onclick="boxOpen('ready-check')">レディチェックが完了</button> ✔${okNum}/${max} ×${noNum}/${max} </dd>`;
    targetTab.appendChild(newLog);
...
function formSubmit(objId,unitName){
  ...
  if(obj.dataset.paletteTarget){
    comm = paletteCheck(unitList[unitName]['id'], name, comm);
  }
  else {
    comm = paletteCheck(selectedSheet, name, comm);
  }
...
// チャットパレット変数チェック
let paletteSetNest;
function paletteCheck(id, name, comm){
  paletteSetNest = 0;
  return comm.replace(/[{｛](.+?)[｝}]/gi, function(raw, varNameRaw){
    return paletteParamSet(id, name, varNameRaw);
  });
}
function paletteParamSet(id, name, varNameRaw){
  const varName = toHalfWidth(varNameRaw).toLowerCase();
  paletteSetNest++;
  if(paletteSetNest >= 100){ return '{'+varNameRaw+'}'; }
  const list = paletteValueList(id,name);
  ...
  for(let i=0; i<list.length; i++){
    if(toHalfWidth(list[i]).match(reg)){
      return RegExp.$1.replace(/[{｛](.+?)[｝}]/gi, function(all, varNameRawChild){
        return paletteParamSet(id, name, varNameRawChild);
      });
    }
  }
  return '{'+varNameRaw+'}';
...
function paletteValueList(id,name){
  let arr = [];
  // ダイス欄
  for(let area in [0,1]){
